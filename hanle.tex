...
Nach den Gleichungen \ref{LK} und \ref{LK0} verhalten sich die Daten für die Polarisationen $Pol=0°$ und $Pol=90°$ entsprechend einer Lorentz-Funktion. Wir fitten an die Daten nun also folgende Funktion:
\begin{align}
...
Die Daten sämtlicher Lorentzfits sind in Tabellenform im Anhang gesammelt (siehe \ref{LF}).
\newpage
\subsection{Dispersionsfit}
Für die Polarisation $Pol=45°$ ist der erwartete Verlauf eine Dispersionskurve (siehe Gleichung \ref{45a} und Abbildung \ref{45}). An diese Daten fitten wir also folgende Funktion:
\begin{align}
...
Die Daten sämtlicher Dispersionsfits sind in Tabellenform im Anhang gesammelt (siehe \ref{DD}).
\newpage
...
\end{align}
Der Fehler auf den Druck berechnet sich aus der Kovarianzmatrix $\sigma$ aus \cite{anleitung} über folgende Formel:
\begin{align*}
